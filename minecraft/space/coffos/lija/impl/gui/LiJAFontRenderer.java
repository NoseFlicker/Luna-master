...
public class LiJAFontRenderer extends FontRenderer {

    private Random fontRandom = new Random();
    private Color[] customColorCodes = new Color[256];
    private int[] colorCode = new int[32];
    private LiJAFont font, boldFont, italicFont, boldItalicFont;
     ...
     */
    public LiJAFontRenderer(Font font, boolean antiAlias,
                            int charOffset) {
                ...
                if (index == 0) {
                    ...
                    if (colorIndex != -1)
                        if (colorIndex < 16) { // coloring
                            ...
                            currentColor = new Color(customColor.getRed() / 255F, customColor.getGreen() / 255F, customColor.getBlue() / 255F, alpha);
                        }

                    if (bold & italic) {
                        boldItalicFont.drawString(randomCase ? toRandom(words) : words, width, 0, currentColor, shadow);
                        currentFont = boldItalicFont;
                    } else if (bold) {
                        boldFont.drawString(randomCase ? toRandom(words) : words, width, 0, currentColor, shadow);
                        currentFont = boldFont;
                    } else if (italic) {
                        italicFont.drawString(randomCase ? toRandom(words) : words, width, 0, currentColor, shadow);
                        currentFont = italicFont;
                    } else {
                        font.drawString(randomCase ? toRandom(words) : words, width, 0, currentColor, shadow);
                        currentFont = font;
                    }
    ...
    private int sizeStringToWidth(String str, int wrapWidth) {
        int var3 = str.length(), var4 = 0, var5 = 0, var6 = -1;
        ...
        if (text.contains("\247")) {
            ...
            for (int index = 0; index < parts.length; index++) {
                ...
                if (index == 0)
                    ...
                    if (colorIndex != -1)
                        // coloring
                        if (colorIndex < 16) bold = italic = false;
                        else if (colorIndex == 17)
                            bold = true;
                        else if (colorIndex == 20)
                            italic = true;
     ...
     */
    private void setFont(Font font, boolean antiAlias,
                         int charOffset) {
        synchronized (this) {
            this.font = new LiJAFont(font, antiAlias, charOffset);
            boldFont = new LiJAFont(font.deriveFont(Font.BOLD), antiAlias,
                    charOffset);
            italicFont = new LiJAFont(font.deriveFont(Font.ITALIC),
                    antiAlias, charOffset);
            boldItalicFont =
                    new LiJAFont(font.deriveFont(Font.BOLD | Font.ITALIC),
                            antiAlias, charOffset);
            FONT_HEIGHT = getHeight();
        }
    }
    ...
    private Color getColor(int colorCode, float alpha) {
